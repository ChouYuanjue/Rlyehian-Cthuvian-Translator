...
function generateEntry(base, commonWords, keywordToRoots, rootGlosses) {
  ...
  const rawRc = selectedRoots.length >= 2 ? rootsToSurface(selectedRoots, rootGlosses) : coinedSurface(base);
  const rc = reserveSurface(rawRc, base, selectedRoots.length < 2);
...
function reserveSurface(surface, base, mayExtend) {
  if (!usedSurfaces.has(surface)) {
    usedSurfaces.set(surface, base);
    return surface;
  }
  if (usedSurfaces.get(surface) === base) return surface;
  if (!mayExtend) return surface;
  let attempt = 1;
  while (attempt < 8) {
    const extension = coinedExtension(base, attempt);
    const candidate = `${surface}${extension}`;
    if (!usedSurfaces.has(candidate)) {
      usedSurfaces.set(candidate, base);
      return candidate;
    }
    attempt += 1;
  }
  return `${surface}${attempt}`;
}
...
function coinedExtension(base, attempt) {
  const hash = fnv1a(`RC-1.0:${base}:${attempt}`);
  return SYLLABLES[hash % SYLLABLES.length];
}
